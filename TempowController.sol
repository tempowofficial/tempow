...
contract TempowController {
    ...
    */
    ...
    function TempowController(address _tokenAddress, uint256 _usdMintCap) public {
        require(_tokenAddress != address(0));
        token = TempowToken(_tokenAddress);
        owner = msg.sender;
        creationBlock = block.number;
        usdMintCap = _usdMintCap;
        tokensMinted = 0;
        usdRaised = 0;
    }
    ...
    */
    ...
    /**
    * @dev Internal function for minting tokens over token contract
    */
    function _mint(address _to, uint256 _value, uint256 _usdValue) internal returns (bool) {
        uint256 newValue = tokensMinted.add(_value);
        require(newValue <= usdMintCap);
        tokensMinted = newValue;
    ...
    */
    function sendTokens(address _to, uint256 _value, uint256 _usdValue) public onlyOwner returns (bool) {
        return _mint(_to, _value, _usdValue);
    }
    ...
    */
    function bulkSendTokens(address[] _tos, uint256[] _values, uint256[] _usdValues) public onlyOwner {
        ...
        for (uint256 i = 0; i < _tos.length; i++) {
            ...
            require(_mint(to, value, usdValue));
        }
    }
